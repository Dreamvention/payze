...
class ControllerExtensionPaymentPayze extends Controller {
	...
	public function confirm() {					
		...
		if ($payze->hasErrors()) {
			...
			foreach ($errors as $error) {
				$this->model_extension_payment_payze->log($error, 'Error');
			}
	...
	public function failurePage() {
		if (!empty($this->request->get['failure_page'])) {
			...
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/payment/payze/failurePage', '', true)
			);
			...
			$this->response->setOutput($this->load->view('extension/payment/payze/failure_page', $data));
		}
	}
	...
	public function webhook() {				
		...
		if (!empty($webhook_info['transactionId']) && !empty($webhook_info['status']) && !empty($this->request->get['authorization_token'])) {
			$this->model_extension_payment_payze->log($webhook_info, 'Webhook');
			...
			if (!empty($result['response']['transactionId']) && !empty($result['response']['status'])) {
				...
				if ($order_status_id) {
					...
					if (!$payze_order_info) {
						...
						$this->model_extension_payment_payze->addOrder($payze_data);
					}
					...
					if ($result['response']['status'] == 'RefundedPartially') {
						...
						if ($order_info) {
							...
							if (isset($result['response']['finalAmount'])) {
								$this->model_extension_payment_payze->setOrderTotal($order_id, $result['response']['finalAmount']);
							}
						}
					}
	...
	public function order_add_order_history_before(&$route, &$data) {
		$this->load->model('extension/payment/payze');
		$this->load->model('checkout/order');
