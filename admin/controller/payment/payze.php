...
class ControllerPaymentPayze extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('payment/payze', 'token=' . $this->session->data['token'], true)
		);
						
		$data['action'] = $this->url->link('payment/payze', 'token=' . $this->session->data['token'], true);
		...
		$this->response->setOutput($this->load->view('payment/payze/payze', $data));
	}
	
	public function install() {		
		$this->load->model('payment/payze');
		
		$this->model_payment_payze->install();
		...
		$this->model_extension_event->addEvent('payze_order_add_history', 'catalog/model/checkout/order/addOrderHistory/before', 'payment/payze/order_add_order_history_before');
		$this->model_extension_event->addEvent('payze_order_delete_order', 'catalog/model/checkout/order/deleteOrder/before', 'payment/payze/order_delete_order_before');
		$this->model_extension_event->addEvent('payze_customer_delete_customer', 'admin/model/customer/customer/deleteCustomer/before', 'payment/payze/customer_delete_customer_before');
	}
		
	public function uninstall() {
		$this->load->model('payment/payze');
		
		$this->model_payment_payze->uninstall();
	...
	public function order() {
		if ($this->config->get('payze_status') && !empty($this->request->get['order_id'])) {
			...
			if ($payze_order_info) {
				...
				return $this->load->view('payment/payze/order', $data);
			}
		}
	}
		
	public function customer_delete_customer_before($route, $customer_id) {
		$this->load->model('payment/payze');

		$this->model_payment_payze->deleteCustomerCards($customer_id);
	}
								
	private function validate() {
		if (!$this->user->hasPermission('modify', 'payment/payze')) {
